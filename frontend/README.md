...
This frontend application is designed for containerized deployment with Kubernetes. See the [k8s directory](../k8s/README.md) for comprehensive deployment instructions using Helm charts.

### Containerization

The frontend is containerized using the Dockerfile in the [docker directory](../docker/Dockerfile.frontend) and can be deployed as part of the complete Todo App stack.

### Environment Variables in Kubernetes

When deployed via Kubernetes, the following environment variables are configured via the Helm chart:

- `NEXT_PUBLIC_API_URL`: Backend API service URL (default: `http://todo-backend-svc:8000`)
